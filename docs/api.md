...
基于分层 Redis 抽象（`RedisOps` 底层操作 → `RedisCache` 中间层缓存 → 业务服务），统一 key 命名空间管理。
...
| `POST` | `/api/redis/expire/{key}?timeout=60` | 设置过期时间（秒） |
| `GET` | `/api/redis/info` | 获取 Redis 服务器信息 |
| `GET` | `/api/redis/stats` | 获取 Redis 统计（含缓存命中率、DB大小、内存信息） |
| `GET` | `/api/redis/keys?pattern=*` | 按模式查询键（SCAN 游标迭代，非阻塞） |
...
| `POST` | `/api/redis/performance/batch-set?count=100` | Pipeline 批量性能测试 |
