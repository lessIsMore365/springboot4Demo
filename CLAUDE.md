...
- **公开端点**（无需认证）：`/api/auth/register`、`/api/auth/captcha`、`/api/auth/captcha/verify`、`/api/auth/health`、`/api/payment/notify/**`、`/api/ai/**`、`/api/mcp/**`、`/api/monitor/db/health`、`/api/monitor/server/health`、`/api/logs/health`、`/.well-known/**`
...
19. **AI Module** — 参考 Pig AI 设计，集成多模型 AI 能力（DeepSeek/通义千问/Kimi/智谱GLM）。所有 AI 端点为公开端点。功能包括：
